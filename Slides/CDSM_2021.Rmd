...
1. What do students tink the data means? Does it depend on context?

2. What model do students assume generated the data? Does it depend on context?
...
Beth Chance, IASE Statelite 2021: Multivariable thinking throughout the curriculum:
...
.pull-left[
Directed Acyclic Graphs (DAG) tell your assumed *data story* - and tell you if you should adjust for $\color{violet}{Z}$ to estimate the total causal effect of $\color{green}{X}$ on $\color{blue}{Y}$.]
...
plot_grid(DAG_fork,DAG_chain, ncol=2, nrow = 1,  label_size=12,
          labels=c('Z and in a fork between X and Y',
                   'Z and in a chain between X and Y'))
```
]
...
.pull-left[
.fom[Ongoing research]: Conclusions may ve even more messy if data does not meet expectations `r fontawesome("sad-tear", style = "solid")`
]
